...
export default function Cart() {
  ...
  let { getAllCartData, deleteProduct, updateProductQuantity, setCartCount } =
    ...
    <div className="bg-light p-4">
      ...
      {CartData?.products.map((el) => {
        return (
          <div
            key={el._id}
            className="row py-2 border-bottom justify-content-between"
          >
      ...
      <h3 className="text-main mt-4">
        Total Pirce :{CartData?.totalCartPrice}
      </h3>

      <Link to={"/CheckOut/" + CartData?._id}>
        <span className="btn btn-success">Check Out</span>
      </Link>
    </div>
  );
}
